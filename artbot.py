...
class CreateSvgApp:
   ...
   def __init__(self, master):
      ...
      self.processImageBtn = Button(controlBtnsFrame, text="Process", justify=LEFT, relief=RAISED, command=self.processFile)
      self.processImageBtn.pack(side=LEFT, fill=BOTH)
        
      self.renderImageBtn = Button(controlBtnsFrame, text="Render", justify=LEFT, relief=RAISED, command=self.renderFile)
      self.renderImageBtn.pack(side=LEFT, fill=BOTH)
        
      self.robotDrawBtn = Button(controlBtnsFrame, text="Send to Robot", justify=LEFT, relief=RAISED, command=self.sendToRobot)
      self.robotDrawBtn.pack(side=LEFT, fill=BOTH)
      
      self.robotPauseBtn = Button(controlBtnsFrame, text="Pause", justify=LEFT, relief=RAISED, command=self.pauseDrawing)
      self.robotPauseBtn.pack(side=LEFT, fill=BOTH)
      
      self.robotResumeBtn = Button(controlBtnsFrame, text="Resume", justify=LEFT, relief=RAISED, command=self.resumeDrawing)
      ...
      self.linRad = Radiobutton(interpFrame, text="Linear", variable=self.interpVar, value=1).pack(side=LEFT, anchor=W)
   ...
   def chooseFile(self):
      picFile = tkFileDialog.askopenfilename(title="Open file", filetypes=[("jpg file",".jpg"),("png file",".png"),("All files",".*")])
   ...
   def processFile(self, fileArg=None):   
      ...
      if self.nb.choice.get() == 0:
         ...
         imgW, imgH, svgW, svgH, lineData = imageParser.createImageData(picFile, workingDir,
                                                                        ...
                                                                        self.interpVar.get(),
                                                                        self.threshVar.get(), self.threshValEnt.get(),
         ...
         imgW, imgH, svgW, svgH, lineData = WTimageParser.createImageData(picFile, workingDir,
                                                                          self.origImgWin, self.procImgWin,
                                                                          self.WTdespecVar.get(), self.WTdespecValEnt.get())
   ...
   def renderFile(self):      
      ...
      self.m_robot = RobotDriver(self, comPort, serialOn, penUpVal, penDownVal, self.m_procSvgW, self.m_procSvgH, self.m_lineData)
